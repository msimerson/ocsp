...
exports.getResponse = function getResponse (uri, req, cb, requestByGet) {
  const { promise, resolve, reject } = Promise.withResolvers()
  promise.then(response => cb(null, response)).catch(err => cb(err))

  let options
  if (requestByGet) {
    options = { method: 'GET' }
    uri = `${uri}/${encodeURIComponent(req.toString('base64'))}`
  } else {
    options = {
      method: 'POST',
      headers: {
  ...
  const onResponse = (response) => {
    if (response.statusCode < 200 || response.statusCode >= 400) {
      return reject(new Error(`Failed to obtain OCSP response: ${response.statusCode}`))
    }
...
exports.parseResponse = function parseResponse (raw) {
  ...
  const certs = (basic.certs ?? []).filter(cert =>
    cert.tbsCertificate.extensions.some(ext => {
      if (ext.extnID !== 'extendedKeyUsage') return false
      return ext.extnValue.some(value => value.join('.') === OCSPSigning)
    })
  )
...
exports.getAuthorityInfo = function getAuthorityInfo (cert, key, done) {
  ...
  let res = null
  const found = infoAccess.some(info => {
